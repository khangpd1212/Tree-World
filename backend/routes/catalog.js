...
router.post('/', verify, async (req, res) => {
  if (req.user.isAdmin) {
    const slugCat = await Catalog.findOne({ slug: req.body.slug });
    if (slugCat) {
      res.status(400).json({ message: 'The slug has been used for another catalog' });
    } else {
...
router.put('/:id', verify, async (req, res) => {
  if (req.user.isAdmin) {
    const slugCat = await Catalog.findOne({ slug: req.body.slug });
    if (slugCat && slugCat._id.toString() !== req.params.id) {
      res.status(400).json({ message: 'The slug has been used for another catalog' });
    } else {
      try {
        ...
        if (!updatedCatalog) throw new Error('Something went wrong with updating catalog');
